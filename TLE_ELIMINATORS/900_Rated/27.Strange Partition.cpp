...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  In order to get the maxi beauty value we should divide each element by x ans them sum it up.
  The mini beauty value we could possible get is by summation of every element of array and then get the ceil value by dividing by x.
...
int main () {
    ...
    while(t--){
        ...
        long long sum = 0;
        for(int i = 0; i < n; i++){
            maxi += (ceil)((double)arr[i]/x);   //each element diviving by x and summation
            sum += arr[i];
        }
        mini = (ceil)((double)sum/x);    //summation everything and then ceil value by dividing by x
        cout<<mini<<" "<<maxi<<endl;    
    }
    return 0;
}
