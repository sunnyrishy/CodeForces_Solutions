...
To solve this, first we need to have a track of number of zeros and ones present in the given string.
  The max moves that is delete operations that we can perform at max will be min(zeros, ones)


To make one delete operation we need 1 zero and 1 one.
In this way say if we have 4 zeros and 5 ones then we can make only 4 moves (A,B,A,B) then Alice will loose.

  moves = min(zeros, ones)

  if moves is even Alice will loose
if moves is off Alive will win
