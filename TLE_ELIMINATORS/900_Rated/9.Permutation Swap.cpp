...
For this problem we need to calculate the max possible value k such that we can swap any pairs of numbers in the given sequence to its right position.

[3,4,1,2] should be aligned as 
[1,2,3,4]

[4,2,6,7,5,3,1]
[1,2,3,4,5,6,7]

Solution to this problem is to find the gcd for each element with its position.
...
int main () {
    ...
    while(t--){
        ...
        int gcd = 0;
        for(int i = 0; i < n; i++){
            gcd = find_gcd(gcd, abs(arr[i] - (i+1)));    //element - its position
        }
        cout<<gcd<<endl;
    }
    return 0;
